...
export const Flag = {
  ...
  // When a provider content-filter / moderation block (or soft model refusal)
  // hits, rephrase+retry this many times before surfacing the error. 0 disables.
  // Default also mirrored in content-filter-recovery.ts (reads env at import).
  // Higher default: open harness redrives same model with rotated reframes.
  MIMOCODE_CONTENT_FILTER_MAX_RECOVERY: number("MIMOCODE_CONTENT_FILTER_MAX_RECOVERY") ?? 12,
