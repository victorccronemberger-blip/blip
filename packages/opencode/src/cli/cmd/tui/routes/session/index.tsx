      ...
      onSelect: async (dialog) => {
        ...
        await DialogPrompt.ask(
          ...
          async (question, active) => {
            const res = await sdk.client.session.ask({ sessionID: route.sessionID, question }).catch((error) => {
              if (active())
                toast.show({
...
function AssistantMessage(props: { message: AssistantMessage; parts: Part[]; last: boolean }) {
  ...
  const verdictMark = createMemo(() => {
    ...
    if (v.paused === "judge_error") return { icon: "!", fg: theme.warning, label: "Judge unavailable · goal kept" }
    if (v.paused === "safety_limit") return { icon: "!", fg: theme.warning, label: "Safety pause · goal kept" }
    if (v.error) return { icon: "!", fg: theme.textMuted, label: "Judge: error" }
