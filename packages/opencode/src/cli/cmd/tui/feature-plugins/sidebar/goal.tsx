...
function View(props: { api: TuiPluginApi; session_id: string }) {
  ...
  const status = createMemo(() => {
    ...
    if (v.paused === "judge_error") return { dot: theme().warning, label: "judge unavailable · goal kept" }
    if (v.paused === "safety_limit") return { dot: theme().warning, label: "safety pause · goal kept" }
    if (v.error) return { dot: theme().textMuted, label: "judge error" }
