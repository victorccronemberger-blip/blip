    ...
    return Service.of({
      environment: Effect.fn("SystemPrompt.environment")(function* (model: Provider.Model, now: number) {
        ...
        const base = [
          ...
          // MIMOCODE_FORCE_LANGUAGE=pt|pt-BR forces Brazilian Portuguese for all
          // user-facing prose (PentesterCode sets this). Otherwise mirror the user.
          process.env.MIMOCODE_FORCE_LANGUAGE?.toLowerCase() === "pt" ||
          process.env.MIMOCODE_FORCE_LANGUAGE?.toLowerCase() === "pt-br"
            ? `IMPORTANT: ALWAYS respond in Brazilian Portuguese (pt-BR). All explanations, status, findings narrative, questions, and summaries MUST be in Portuguese. Keep code, shell commands, payloads, HTTP headers, file paths, and technical identifiers unchanged (English identifiers are fine). Never switch to English for user-facing text unless the user explicitly asks for English.`
