...
import {
  ...
  strategyForAttempt,
} from "../session/prompt/content-filter-recovery"
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const runLoop: (
      ...
      // Provider content-filter / soft-refusal reformulation attempts this turn.
      // Each hit discards the blocked assistant turn and injects a synthetic
      // user nudge; after CONTENT_FILTER_MAX_RECOVERY, surface the error.
      let contentFilterRecoveryAttempts = 0
      ...
      return yield* Effect.gen(function* () {
        ...
        // re-entry, which only fires for spawned actors. Judge failures pause
        // the current run but keep the goal active; infrastructure failure must
        // never be misreported as goal completion.
        const goalGate = Effect.fn("SessionPrompt.goalGate")(function* (lastUser: MessageV2.User) {
          ...
          const judged = yield* goal
            ...
            .pipe(
              ...
              Effect.map((verdict) => ({ verdict }) as const),
              Effect.catch((err) =>
                Effect.gen(function* () {
                  yield* slog.warn("goal judge failed; pausing with goal active", { error: String(err) })
                  return { judgeError: err } as const
                }),
              ),
            )
          ...
          if (current?.revision !== active.revision) {
            yield* slog.info("discarding stale goal verdict", { sessionID })
            return false
          }

          if ("judgeError" in judged) {
            yield* bus.publish(Goal.Event.Updated, {
              sessionID,
              goal: { condition: active.condition },
              lastVerdict: {
                ok: false,
                reason: "The goal judge is temporarily unavailable. The goal remains active and can be resumed safely.",
          ...
          if (verdict.ok || verdict.impossible) {
            if (
              !(yield* goal.finish(sessionID, active.revision, {
                ...verdict,
                attempt: active.react,
                messageID: judgedMessageID,
              }))
          ...
          const count = yield* goal.bumpReact(sessionID, active.revision)
          if (count === undefined) return false
          if (count > MAX_GOAL_REACT) {
            yield* slog.warn("goal hit MAX_GOAL_REACT cap; pausing with goal active", {
              sessionID,
              count,
            })
            if ((yield* goal.get(sessionID))?.revision !== active.revision) return false
            yield* bus.publish(Goal.Event.Updated, {
              sessionID,
              goal: { condition: active.condition },
              lastVerdict: {
                ...verdict,
                reason: `Safety pause after ${count} goal rounds. The goal remains active; continue or use /goal clear.`,
                attempt: count,
                messageID: judgedMessageID,
                error: true,
                paused: "safety_limit",
        ...
        // content-filter / moderation / soft-refusal recovery.
        // Discard the blocked assistant turn (error flag → skipped by toModelMessages),
        // inject a synthetic reformulation nudge, and continue the loop. After
        // CONTENT_FILTER_MAX_RECOVERY attempts, surface a terminal error.
        const writeContentFilterError = Effect.fn("SessionPrompt.writeContentFilterError")(function* (input: {
          assistant: MessageV2.Assistant
        }) {
          if (input.assistant.error?.name === "ContentFilterError") {
            yield* bus.publish(Session.Event.Error, {
              sessionID: input.assistant.sessionID,
              error: input.assistant.error,
            })
        ...
        const handleContentFilterRecovery = Effect.fn("SessionPrompt.handleContentFilterRecovery")(function* (input: {
          lastUser: MessageV2.User
          assistant: MessageV2.Assistant
          reason: "provider-filter" | "soft-refusal" | "api-moderation"
        }) {
          if (CONTENT_FILTER_MAX_RECOVERY <= 0) {
            yield* writeContentFilterError({ assistant: input.assistant })
            return false
          }
          // Discard blocked/refused turn from request history so the next
          // generation is not poisoned by the refusal text or empty filter body.
          if (!input.assistant.error) {
            input.assistant.error = new MessageV2.ContentFilterError({
              message:
                input.reason === "soft-refusal"
                  ? "Soft refusal detected; discarded for content-filter recovery."
                  : "The response was withheld by the model provider's content safety filter.",
            }).toObject()
            yield* sessions.updateMessage(input.assistant)
          } else if (input.assistant.error.name !== "ContentFilterError") {
            // Keep APIError details but still recover; tag for visibility.
            yield* sessions.updateMessage(input.assistant)
          }

          if (contentFilterRecoveryAttempts >= CONTENT_FILTER_MAX_RECOVERY) {
            yield* slog.info("content-filter: max recovery exceeded", {
              attempts: contentFilterRecoveryAttempts,
              reason: input.reason,
            })
            yield* writeContentFilterError({ assistant: input.assistant })
            return false
          ...
          // Pull the real operator task (non-synthetic) from the nearest user turn
          // so each strategy can restate it under a different framing.
          const lastUserWithParts = (yield* MessageV2.filterCompactedEffect(sessionID, {
            contextFrom: session.contextFrom,
            contextWatermark: session.contextWatermark,
            agentID: agentID ?? "main",
          })).findLast((m) => m.info.role === "user" && m.info.id === input.lastUser.id)
          const originalTask = lastUserWithParts ? extractOriginalTask(lastUserWithParts.parts) : ""

          const attempt = contentFilterRecoveryAttempts
          const strategy = strategyForAttempt(attempt)
          const recoveryText = recoveryPromptForAttempt(attempt, originalTask)

          // Same-model by default; model rotation only if ALLOW_MODEL_SWITCH=1.
          const nextModel = modelForRecoveryAttempt(attempt, {
            providerID: input.lastUser.model.providerID,
            modelID: input.lastUser.model.modelID,
          })
          const modelChanged =
            nextModel.providerID !== input.lastUser.model.providerID ||
            nextModel.modelID !== input.lastUser.model.modelID
          ...
          const reentry = yield* sessions.updateMessage({
            ...
            agent: input.lastUser.agent,
            model: {
              providerID: ProviderID.make(nextModel.providerID),
              modelID: ModelID.make(nextModel.modelID),
          ...
          yield* sessions.updatePart({
            id: PartID.ascending(),
            messageID: reentry.id,
            sessionID: reentry.sessionID,
            type: "text",
            synthetic: true,
            text: recoveryText,
          } satisfies MessageV2.TextPart)
          contentFilterRecoveryAttempts++
          yield* slog.info("content-filter: recovery injected", {
            attempt: contentFilterRecoveryAttempts,
            reason: input.reason,
            strategy: strategy.id,
            max: CONTENT_FILTER_MAX_RECOVERY,
            model: `${nextModel.providerID}/${nextModel.modelID}`,
            modelChanged,
          })
          return true
        })
        ...
        while (true) {
          ...
          if (lastAssistant) {
            ...
            if (classification.type === "filtered") {
              if (
                yield* handleContentFilterRecovery({
                  lastUser,
                  assistant: lastAssistant,
                  reason: "provider-filter",
                })
              )
                continue
            ...
            if (classification.type === "failed") {
              if (
                isProviderContentBlock({ finish: lastAssistant.finish, error: lastAssistant.error }) &&
                (yield* handleContentFilterRecovery({
                  lastUser,
                  assistant: lastAssistant,
                  reason: "api-moderation",
                }))
              )
                continue
            ...
            if (classification.type === "final" && !lastAssistant.error) {
              const finalText = (lastAssistantMsg?.parts ?? [])
                .filter((p): p is MessageV2.TextPart => p.type === "text" && !p.synthetic && !p.ignored)
                .map((p) => p.text)
                .join("\n")
              const userTask = extractOriginalTask(MessageV2.parts(lastUser.id))
              if (
                looksLikeSoftRefusal(finalText, { userTask }) &&
                (yield* handleContentFilterRecovery({
                  lastUser,
                  assistant: lastAssistant,
                  reason: "soft-refusal",
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            if (isForkAgent) {
              ...
              if (forkClassification.type === "failed") {
                if (
                  isProviderContentBlock({ finish: handle.message.finish, error: handle.message.error }) &&
                  (yield* handleContentFilterRecovery({
                    lastUser,
                    assistant: handle.message,
                    reason: "api-moderation",
              ...
              if (forkClassification.type === "final" && !handle.message.error) {
                const forkFinalText = MessageV2.parts(handle.message.id)
                  .filter((p): p is MessageV2.TextPart => p.type === "text" && !p.synthetic && !p.ignored)
                  .map((p) => p.text)
                  .join("\n")
                const userTask = extractOriginalTask(MessageV2.parts(lastUser.id))
                if (
                  looksLikeSoftRefusal(forkFinalText, { userTask }) &&
                  (yield* handleContentFilterRecovery({
                    lastUser,
                    assistant: handle.message,
                    reason: "soft-refusal",
            ...
            if (classification.type === "final" && !handle.message.error) {
              const finalText = MessageV2.parts(handle.message.id)
                .filter((p): p is MessageV2.TextPart => p.type === "text" && !p.synthetic && !p.ignored)
                .map((p) => p.text)
                .join("\n")
              const userTask = extractOriginalTask(MessageV2.parts(lastUser.id))
