...
export function parseStreamError(input: unknown): ParsedStreamError | undefined {
  ...
  const error = body.type === "error" ? body.error : body.type === "response.failed" ? body.response?.error : undefined
  ...
  switch (error.code ?? error.type) {
    ...
    case "server_error":
      return {
        type: "api_error",
        message: typeof error.message === "string" ? error.message : "Provider server error",
        isRetryable: true,
        responseBody,
      }
  }
