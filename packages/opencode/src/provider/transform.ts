...
/** GPT-5.6 Sol/Terra/Luna (+ alias) support max effort in the API and Codex. */
function isGpt56Family(id: string) {
  return id.includes("5.6")
}
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  switch (model.api.npm) {
    case "@openrouter/ai-sdk-provider":
      ...
      {
        const efforts = isGpt56Family(id) ? ["none", ...WIDELY_SUPPORTED_EFFORTS, "xhigh", "max"] : OPENAI_EFFORTS
    ...
    case "@ai-sdk/github-copilot":
      ...
      const copilotEfforts = iife(() => {
        if (
          ...
          const arr = [...WIDELY_SUPPORTED_EFFORTS, "xhigh"]
          if (isGpt56Family(id)) arr.push("max")
          return arr
        }
    ...
    case "@ai-sdk/openai":
      ...
      // GPT-5.6 efforts: none | low | medium | high | xhigh | max (no minimal).
      ...
      const openaiEfforts = iife(() => {
        ...
        const arr = [...WIDELY_SUPPORTED_EFFORTS]
        // GPT-5.6 family does not advertise "minimal" in models.dev / API docs.
        if (!isGpt56Family(id) && (id.includes("gpt-5-") || id === "gpt-5")) {
          arr.unshift("minimal")
        }
        if (model.release_date >= "2025-11-13" || isGpt56Family(id)) {
          arr.unshift("none")
        }
        if (model.release_date >= "2025-12-04" || isGpt56Family(id)) {
          arr.push("xhigh")
        }
        if (isGpt56Family(id)) {
          arr.push("max")
