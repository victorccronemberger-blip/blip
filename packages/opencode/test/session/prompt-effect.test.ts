...
function makeHttp(mcpService = mcp, goalService = Goal.defaultLayer) {
...
let failedJudgeClearCalls = 0
const failedJudgeIt = testEffect(
  makeHttp(
    mcp,
    Layer.succeed(
      Goal.Service,
      Goal.Service.of({
        set: () => Effect.void,
        get: () => Effect.succeed({ condition: "tests pass", react: 0, revision: 1 }),
        clear: () => Effect.sync(() => (failedJudgeClearCalls++, true)),
        finish: () => Effect.sync(() => (failedJudgeClearCalls++, true)),
        bumpReact: () => Effect.succeed(1),
        evaluate: () => Effect.promise(() => Promise.reject(new Error("judge unavailable"))),
      }),
    ),
  ),
)
...
failedJudgeIt.live("keeps an active goal when the judge is unavailable", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* () {
      failedJudgeClearCalls = 0
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const chat = yield* sessions.create({ title: "Pinned" })
      yield* seed(chat.id, { finish: "stop" })
      ...
      const result = yield* prompt.loop({ sessionID: chat.id })
      expect(result.info.role).toBe("assistant")
      expect(failedJudgeClearCalls).toBe(0)
    }),
    { git: true, config: providerCfg },
  ),
)
