...
describe("session.retry.retryable", () => {
  test("retries Codex SSE server_error events", () => {
    const error = MessageV2.fromError(
      {
        ...
        error: {
          ...
          code: "server_error",
          message:
            "An error occurred while processing your request. You can retry your request. Request ID: test-request-id",
          param: null,
        },
      },
    ...
    expect(SessionRetry.retryable(error)).toContain("You can retry your request")
  })

  test("retries Codex response.failed server errors", () => {
    const error = MessageV2.fromError(
      {
    ...
    expect(MessageV2.APIError.isInstance(error)).toBe(true)
    expect(SessionRetry.retryable(error)).toContain("test-response-id")
  })
