...
const opencodeClient = process.env.MIMOCODE_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opencodeClient === undefined) delete process.env.MIMOCODE_CLIENT
  else process.env.MIMOCODE_CLIENT = opencodeClient
})
