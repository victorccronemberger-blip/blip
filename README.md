...
```bash
# Build Windows and Linux binaries and package each one with an offline installer
bun run build:installers

# Build and package only the current operating system and architecture
bun run --cwd packages/opencode script/build.ts --single --package
```

Artifacts are written to `packages/opencode/dist/` as Windows `.zip` and Linux `.tar.gz` files.
After extracting an artifact, install it locally:

```powershell
# Windows
.\install.ps1 -Binary .\mimo.exe
```
